...
diferenca = datetime.timedelta(hours=-3) #nao consegui instalar o pytz na AWS, tenho que arrumar isso
...
# Ja essa parte traz funcoes que serao usadas para acessar o site da fump, pegar o cardapio, trata-lo e por fim, posta-lo no Twitter
...
def pega_cardapio(site_fump, restaurante, almoco, jantar):
    ...
    # o cardapio para todos os restaurantes fica na mesma pagina no site da Fump. Alguma coisa em JS acontece por tras de uma
    # caixa de selecao de restaurantes e, somente a partir dai, o conteudo dos cardapios pode ser acessado
    seleciona_caixa_de_selecao(restaurante)

    # procurando e retornando os itens do cardapio desejado, i.e., almoco ou jantar (afinal, o restaurante ja foi escolhido)
...
def seleciona_caixa_de_selecao(restaurante):
    ...
    try:        
        if restaurante == RU_SETORIAL_I:
            ...
            caixa_de_selecao.send_keys(Keys.ENTER)      # vez nao estava funcionando. Nao sei se essa e' uma limitacao ou bug da
            ...
            caixa_de_selecao.click()                    # descobri que apertar a tecla R navegava pelo menu. E' exatamente isso que
...
def confere_cardapio(cardapio):
    ...
    for letra in cardapio:
        count_caracteres += 1
    #confere se ele e' tuitavel, ie, tem menos de 280 caracteres
    if (count_caracteres <= 280):
        mais_280_caracteres = False
    else:
        mais_280_caracteres = True

    return mais_280_caracteres

    #a ideia e' definir outros tipos de possiveis erros aqui nessa funcao e retorna-los todos juntos
'''
...
def faz_tweet(restaurante, cardapio):
    ...
    if (count_caracteres <= 280): # nao tem problema tuitar
        mais_280_caracteres = False
    else:                          # nao e' tuitavel e por isso precisa ser dividido
        mais_280_caracteres = True
...
def acessa_e_tweeta(almoco, jantar, dia_da_semana, restaurantes_almoco_seg_sex, restaurantes_almoco_sab, restaurantes_jantar_seg_sab):
    ...
    if almoco:
        ...
        if dia_da_semana < 5: # se nao e' fim de semana
        ...
        elif dia_da_semana == 5: # se e' sabado
        ...
        if dia_da_semana != 6: # se nao e' domingo
...
aux_postagem_almoco = datetime.datetime.now().astimezone(fuso_horario)
horario_postagem_almoco = aux_postagem_almoco.replace(hour=9, minute=0, second=0, microsecond=0)
'''
aux_limite_almoco = datetime.datetime.now()
horario_limite_postagem_almoco = aux_limite_almoco.replace(hour=9, minute=5, second=0, microsecond=0)
'''

aux_postagem_jantar = datetime.datetime.now().astimezone(fuso_horario)
horario_postagem_jantar = aux_postagem_jantar.replace(hour=17, minute=0, second=0, microsecond=0)
'''
aux_limite_jantar = datetime.datetime.now()
horario_limite_postagem_jantar = aux_limite_jantar.replace(hour=17, minute=5, second=0, microsecond=0)


aux_meia_noite = datetime.datetime.now()
meia_noite = aux_meia_noite.replace(hour=0, minute=0, second=0, microsecond=0)
'''

horario_atual = datetime.datetime.now().astimezone(fuso_horario)

if horario_atual >= horario_postagem_almoco and horario_atual < horario_postagem_jantar:
    almoco = True
    jantar = False

elif horario_atual >= horario_postagem_jantar:
    almoco = False
    jantar = True



acessa_e_tweeta(almoco, jantar, dia_da_semana, restaurantes_almoco_seg_sex, restaurantes_almoco_sab, restaurantes_jantar_seg_sab)
...
if len(restaurantes_nao_tweetados) > 0: # sei que essa forma de comparacao nao e' Pythonic, mas achei assim melhor fazer essa checagem de forma explicita
